...
echo "<script>console.log(" . ($_SESSION['user']['parent_id']) . ")</script>";

$parent = mysqli_fetch_assoc(mysqli_query($conn, $query));
echo "<script>console.table(" . json_encode($parent) . ")</script>";

$classroom = $query3;
$grade = mysqli_fetch_assoc(mysqli_query($conn, $query4));

?>
<div class=" flex flex-col items-center justify-between translate-y-28 md:translate-x-52 mb-10 mx-34 w-[80%] overflow-x-hidden">
